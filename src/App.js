...
// Google Sheets Integration - Replace with your deployed Apps Script URL
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {step === 1 && (
          <div className="bg-white rounded-xl p-6 space-y-4">
            ...
            <div>
              ...
              <input type="text" value={demographics.studentId} 
                onChange={e => setDemographics({ ...demographics, studentId: e.target.value })}
                className="w-full px-3 py-2 border-2 rounded-lg" placeholder="Enter your student ID" />
            </div>
            
            <div>
              <label className="block text-sm font-medium mb-1">Your Name *</label>
              <input type="text" value={demographics.name}
                onChange={e => setDemographics({ ...demographics, name: e.target.value })}
                className="w-full px-3 py-2 border-2 rounded-lg" placeholder="Enter your name" />
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {['Undergraduate', "Master's", 'PhD', 'Other'].map(opt => (
                  <label key={opt} className={`block p-3 rounded-lg border-2 cursor-pointer ${demographics.program === opt ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                    <input type="radio" name="program" value={opt} checked={demographics.program === opt}
                      onChange={e => setDemographics({ ...demographics, program: e.target.value })} className="mr-2" />
                    {opt}
                  </label>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {['Construction Science', 'Construction Management', 'Civil Engineering', 'Other'].map(opt => (
                  <label key={opt} className={`block p-3 rounded-lg border-2 cursor-pointer ${demographics.major === opt ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                    <input type="radio" name="major" value={opt} checked={demographics.major === opt}
                      onChange={e => setDemographics({ ...demographics, major: e.target.value })} className="mr-2" />
                    {opt}
                  </label>
                ))}
              </div>
              ...
              <div className="flex gap-4">
                {['Yes', 'No'].map(opt => (
                  <label key={opt} className={`flex-1 p-3 rounded-lg border-2 cursor-pointer text-center ${demographics.priorCourses === opt ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                    <input type="radio" name="priorCourses" value={opt} checked={demographics.priorCourses === opt}
                      onChange={e => setDemographics({ ...demographics, priorCourses: e.target.value })} className="mr-2" />
                    {opt}
                  </label>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  { v: 'never', l: 'Never heard of it' },
                  { v: 'heard', l: 'Heard of it but never used it' },
                  { v: 'class', l: 'Used it in class/assignments' },
                  { v: 'work', l: 'Used it in real projects' }
                ].map(opt => (
                  <label key={opt.v} className={`block p-3 rounded-lg border-2 cursor-pointer ${demographics.lobFamiliarity === opt.v ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                    <input type="radio" name="lobFamiliarity" value={opt.v} checked={demographics.lobFamiliarity === opt.v}
                      onChange={e => setDemographics({ ...demographics, lobFamiliarity: e.target.value })} className="mr-2" />
                    {opt.l}
                  </label>
                ))}
              </div>
            ...
            <button onClick={() => setStep(2)} disabled={!isDemoComplete}
              className={`w-full py-3 rounded-lg font-bold text-lg ${isDemoComplete ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
        ...
        {step === 2 && (
          <div className="bg-white rounded-xl p-6 space-y-4">
            ...
            <p className="text-sm text-gray-600">Select the best answer for each question. It's okay if you're not sure - just give your best guess!</p>
            ...
            {KNOWLEDGE_QUESTIONS.map((q, idx) => (
              <div key={q.id} className="border rounded-lg p-4">
                ...
                <div className="space-y-2">
                  {q.options.map(opt => (
                    <label key={opt.value} className={`block p-3 rounded-lg border-2 cursor-pointer ${knowledge[q.id] === opt.value ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                      <input type="radio" name={q.id} value={opt.value} checked={knowledge[q.id] === opt.value}
                        onChange={e => setKnowledge({ ...knowledge, [q.id]: e.target.value })} className="mr-2" />
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setStep(3)} disabled={!isKnowledgeComplete}
                className={`flex-1 py-3 rounded-lg font-bold ${isKnowledgeComplete ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
                Continue →
              </button>
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="bg-white rounded-xl p-6 space-y-4">
                ...
                <div className="flex justify-between gap-2">
                  {[1, 2, 3, 4, 5].map(n => (
                    <label key={n} className={`flex-1 flex flex-col items-center cursor-pointer p-3 rounded-lg border-2 ${selfEfficacy[q.id] === n ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                      <input type="radio" className="sr-only" checked={selfEfficacy[q.id] === n}
                        onChange={() => setSelfEfficacy({ ...selfEfficacy, [q.id]: n })} />
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSubmit} disabled={!isSEComplete || submitting}
                className={`flex-1 py-3 rounded-lg font-bold ${isSEComplete && !submitting ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-800 to-green-600 p-4">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Step 1: Knowledge (same questions as pre-survey) */}
        {step === 1 && (
          <div className="bg-white rounded-xl p-6 space-y-4">
            ...
            {KNOWLEDGE_QUESTIONS.map((q, idx) => (
              <div key={q.id} className="border rounded-lg p-4">
                ...
                <div className="space-y-2">
                  {q.options.map(opt => (
                    <label key={opt.value} className={`block p-3 rounded-lg border-2 cursor-pointer ${knowledge[q.id] === opt.value ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-green-300'}`}>
                      <input type="radio" name={`post-${q.id}`} value={opt.value} checked={knowledge[q.id] === opt.value}
                        onChange={e => setKnowledge({ ...knowledge, [q.id]: e.target.value })} className="mr-2" />
                      <span className="font-medium">{opt.value.toUpperCase()})</span> {opt.label}
                    </label>
                  ))}
            ...
            <button onClick={() => setStep(2)} disabled={!isKnowledgeComplete}
              className={`w-full py-3 rounded-lg font-bold ${isKnowledgeComplete ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
              Continue →
            </button>
          </div>
        )}

        {/* Step 2: Self-Efficacy (same questions as pre-survey) */}
        {step === 2 && (
          <div className="bg-white rounded-xl p-6 space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(1)} className="flex-1 py-3 rounded-lg font-bold border-2 hover:bg-gray-50">← Back</button>
              <button onClick={() => setStep(3)} disabled={!isSEComplete}
                className={`flex-1 py-3 rounded-lg font-bold ${isSEComplete ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
                Continue →
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 space-y-4">
                ...
                <div className="flex justify-between gap-2">
                  {[1, 2, 3, 4, 5].map(n => (
                    <label key={n} className={`flex-1 flex flex-col items-center cursor-pointer p-3 rounded-lg border-2 ${experience[q.id] === n ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-green-300'}`}>
                      <input type="radio" className="sr-only" checked={experience[q.id] === n}
                        onChange={() => setExperience({ ...experience, [q.id]: n })} />
            ...
            <div>
              ...
              <textarea value={comments} onChange={e => setComments(e.target.value)}
                className="w-full px-3 py-2 border-2 rounded-lg" rows={4}
                placeholder="What did you like? What could be improved? Any suggestions?" />
            </div>

            <div className="flex gap-3">
              <button onClick={() => setStep(2)} className="flex-1 py-3 rounded-lg font-bold border-2 hover:bg-gray-50">← Back</button>
              <button onClick={handleSubmit} disabled={!isEXComplete || submitting}
                className={`flex-1 py-3 rounded-lg font-bold ${isEXComplete && !submitting ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
...
// ==================== GAME COMPONENTS (from your current code) ====================

// Draggable Bar Chart for R1
        ...
        <div ref={chartRef} className="relative bg-gray-50 rounded-lg border" style={{ width: CHART_WIDTH, height: CHART_HEIGHT, margin: '0 auto', minWidth: CHART_WIDTH, touchAction: 'pan-x' }}>
          ...
          {bars.map((bar, index) => (
            bar.show ? (
              ...
              <div key={bar.id} className="absolute bg-gray-200 rounded flex items-center justify-center text-gray-400 text-xs border-2 border-dashed border-gray-300"
...
function Round1({ onComplete }) {
  ...
  const InputCell = ({ value, onChange, disabled }) => (
    <input type="number" value={value} onChange={onChange} disabled={disabled}
      className={`w-16 px-1 py-1 border-2 rounded text-center text-sm font-bold ${disabled ? 'bg-gray-100 border-gray-300 text-gray-400' : 'bg-yellow-50 border-yellow-400'}`}
      placeholder="?" />
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-5">
        ...
        <table className="w-full text-sm border">
          <thead className="bg-gray-100">
            <tr><th className="px-2 py-2 border text-left">Activity</th><th className="px-2 py-2 border text-center">Rate (ft/day)</th><th className="px-2 py-2 border text-center bg-yellow-50">Duration (days)</th></tr>
          </thead>
          <tbody>
      ...
      {durValidated && allDurationsCorrect && (
        <div className="bg-white rounded-lg shadow p-5">
          ...
          <table className="w-full text-sm border">
            <thead className="bg-gray-100">
              <tr><th className="px-2 py-2 border text-left">Activity</th><th className="px-2 py-2 border text-center">Rate (ft/day)</th><th className="px-2 py-2 border text-center">Duration (days)</th><th className="px-2 py-2 border text-center bg-yellow-50">Start</th><th className="px-2 py-2 border text-center">End</th></tr>
            </thead>
            <tbody>
      ...
      {durValidated && allDurationsCorrect && (
        <div className="bg-white rounded-lg shadow p-5">
          ...
          <DraggableBarChart 
            schedule={{ excStart: fullSchedule.excS, pipeStart: fullSchedule.pipeS, backStart: fullSchedule.backS }}
            durations={{ exc: CORRECT_DURATIONS.exc, pipe: CORRECT_DURATIONS.pipe, back: CORRECT_DURATIONS.back }}
            onScheduleChange={handleBarDrag}
          />
        </div>
      )}

      <button onClick={() => onComplete(fullSchedule)} disabled={!allScheduleFilled || !allDurationsCorrect}
    ...
    <div className="space-y-2">
      {bars.map((bar, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <div className="flex-1 h-6 bg-gray-100 rounded relative">
            <div className={`absolute h-full ${bar.color} rounded flex items-center justify-center text-white text-xs`}
              style={{ left: `${dayToPercent(bar.start)}%`, width: `${Math.max(dayToPercent(bar.end - bar.start + 1), 5)}%` }}>
              {bar.start}-{bar.end}
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex justify-between text-xs text-gray-500 px-1">
          <span>0</span><span>{Math.round(maxDay/4)}</span><span>{Math.round(maxDay/2)}</span><span>{Math.round(maxDay*3/4)}</span><span>{maxDay}</span>
        </div>
      </div>
...
function LOBChartR1({ schedule, durations }) {
  ...
  const getDistAtDay = (start, end, day, rate) => {
    ...
    return Math.min(Math.round(rate * (day - start + 1)), PROJECT_LENGTH);
  };
  ...
  return (
    <div className="relative">
      ...
      {hover && (
        <div className="absolute bg-white border border-gray-300 rounded shadow-lg p-2 text-xs pointer-events-none z-10" style={{ left: Math.min(hover.x + 10, CHART_WIDTH - 160), top: Math.max(hover.y - 80, 5) }}>
          <div className="font-bold mb-1">Day {hover.day}</div>
          {lines.map(l => (<div key={l.id} className="flex justify-between gap-3" style={{ color: l.color }}><span>{l.name.split(' ')[0]}:</span><span className="font-mono">{getDistAtDay(l.start, l.end, hover.day, l.rate).toLocaleString()} ft</span></div>))}
        </div>
      )}
      ...
      <div className="mt-2 text-center">
        {conflicts.length > 0 ? (
          <div>
            <span className="inline-block px-2 py-1 bg-red-100 text-red-700 text-xs rounded font-medium">❌ {conflicts.length} conflict{conflicts.length > 1 ? 's' : ''} detected!</span>
            <div className="mt-1 text-xs text-red-600">{conflicts.map((c, i) => (<div key={i}>Day {c.day}: {c.bName} passes {c.aName} at {c.dist.toLocaleString()} ft</div>))}</div>
          </div>
        ...
        <div className="lg:w-64 space-y-3">
          ...
          <div className={`rounded-lg p-3 text-sm ${r2Conflicts.length === 0 ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
            ...
            {r2Conflicts.length > 0 && r2Conflicts.slice(0, 2).map((c, i) => (<div key={i} className="text-xs text-red-600">Day {c.day}: {c.bName.split(' ')[0]} passes {c.aName.split(' ')[0]}</div>))}
          </div>
        </div>
      </div>
...
export default function LOBGame() {
  ...
  const [sessionId] = useState(() => 'S' + Date.now().toString(36) + Math.random().toString(36).substr(2, 5));
  ...
  const [r2Schedule, setR2Schedule] = useState({ excS: MOB_DAYS + 1, pipeS: MOB_DAYS + 1, backS: MOB_DAYS + 1 });
  ...
  // Durations
  const dur = useMemo(() => CORRECT_DURATIONS, []);
  ...
  const r2Full = useMemo(() => ({
    ...
    end: Math.max(r2Schedule.excS + dur.exc - 1, r2Schedule.pipeS + dur.pipe - 1, r2Schedule.backS + dur.back - 1)
  }), [r2Schedule, dur]);

  // R2 validation
  const r2BuffersOk = (r2Schedule.pipeS - r2Schedule.excS === DEFAULT_BUFFER) && 
    ((r2Schedule.backS + dur.back - 1) - (r2Schedule.pipeS + dur.pipe - 1) === DEFAULT_BUFFER);
  const r2Conflicts = findConflicts(r2Full, dur);
  const r2Valid = r2BuffersOk && r2Conflicts.length === 0;
  ...
  // Handle round transitions and save results
  ...
  const handleR2Complete = () => {
    setGameResults(prev => ({ ...prev, r2: { ...r2Full, cost: r2Cost.total } }));
    setRound(4);
  };

  const handleR3Complete = () => {
    setGameResults(prev => ({ ...prev, r3: { ...r3, buffer: r3Buffer } }));
    setRound(5);
  };

  const handleR4Complete = () => {
    setGameResults(prev => ({ ...prev, r4: { end: r4.end, cost: r4Cost.total, equipment: r4Eq } }));
    setRound(6);
  };
  ...
  const handleR5Complete = async () => {
    ...
    await submitToGoogleSheets('game-results', {
      sessionId,
      timestamp: new Date().toISOString(),
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-900 to-blue-700 p-4">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="bg-white rounded-xl p-5">
            ...
            <p className="text-gray-600 mb-4">Before starting the game, you'll complete a short survey to help us understand your background.</p>
            <button onClick={() => setRound(1)} className="w-full bg-blue-600 text-white py-4 rounded-lg font-bold text-lg hover:bg-blue-700">
              Begin Survey →
            </button>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-900 to-blue-700 p-4">
        <div className="max-w-4xl mx-auto bg-white rounded-xl p-6 space-y-6">
          ...
          <div className={`p-4 rounded-lg ${r5Pass ? 'bg-green-100 border-2 border-green-500' : 'bg-yellow-100 border-2 border-yellow-500'}`}>
            <h3 className="font-bold text-lg">{r5Pass ? '✅ Constraints Met!' : '⚠️ Constraints Not Met'}</h3>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <table className="w-full text-sm border">
              <thead className="bg-gray-100">
                <tr><th className="px-3 py-2 border text-left">Round</th><th className="px-3 py-2 border text-center">Duration</th><th className="px-3 py-2 border text-center">Cost</th><th className="px-3 py-2 border text-center">Notes</th></tr>
              </thead>
              <tbody>
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-start gap-2">
                ...
                <div><strong>Activity Sequence Matters:</strong> In linear projects like pipelines, crews must work in sequence (Excavation → Pipe Laying → Backfill). One crew cannot pass another without causing conflicts.</div>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-blue-500 font-bold">2.</span>
                <div><strong>LOB Shows Conflicts:</strong> Line of Balance charts visualize crew progress over distance. When lines cross, it indicates a conflict where crews would be at the same location simultaneously.</div>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-blue-500 font-bold">3.</span>
                <div><strong>Buffers Prevent Conflicts:</strong> Adding buffer days between activities creates safety margin. Larger buffers = safer schedule but longer duration. Buffer doesn't affect cost directly.</div>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-blue-500 font-bold">4.</span>
                <div><strong>Production Rate Trade-offs:</strong> Faster equipment reduces duration but increases daily cost. The optimal solution balances speed and cost to meet project constraints.</div>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-blue-500 font-bold">5.</span>
                <div><strong>Duration vs Cost:</strong> These are often competing objectives. Meeting both constraints requires careful optimization of equipment selection and buffer sizing.</div>
              </div>
            </div>
          </div>

          <button onClick={() => setRound(8)} className="w-full bg-green-600 text-white py-4 rounded-lg font-bold text-lg hover:bg-green-700">
  ...
  if (round === 8) {
    return (
      <PostSurvey
        sessionId={sessionId}
        playerName={preSurveyData?.demographics?.name}
        studentId={preSurveyData?.demographics?.studentId}
        preKnowledgeScore={preSurveyData?.knowledgeScore}
        preSEScore={preSurveyData?.seScore}
        onComplete={(data) => {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-5xl mx-auto p-4 space-y-4">
        ...
        {/* ROUND 2: R1 BAR CHART */}
        {round === 2 && <Round1 onComplete={handleR1Complete} />}

        {/* ROUND 3: R2 LOB ANALYSIS */}
        {round === 3 && r1Schedule && (<>
          ...
          <div className="bg-white rounded-lg shadow p-4 space-y-3">
            ...
            <FlashCard title="What is Line of Balance (LOB)?" icon="📊" isOpen={openCards.lob} onToggle={() => toggleCard('lob')}>
              <p>LOB is a scheduling technique for repetitive or linear projects. Each line shows a crew's progress over time (x-axis) vs. distance/location (y-axis). The slope indicates production rate—steeper = faster.</p>
            </FlashCard>
            <FlashCard title="How to identify conflicts?" icon="⚠️" isOpen={openCards.conflict} onToggle={() => toggleCard('conflict')}>
              <p>A conflict occurs when lines cross—meaning two crews would be at the same location at the same time. The red ✕ markers show where conflicts exist.</p>
            </FlashCard>
            <FlashCard title="What are buffers and why use them?" icon="🛡️" isOpen={openCards.buffer} onToggle={() => toggleCard('buffer')}>
              <p>Buffers are safety margins (in days) between activities. They prevent crews from getting too close and provide contingency for delays. Standard practice is {DEFAULT_BUFFER}-day buffer.</p>
            </FlashCard>
            <FlashCard title="Buffer formulas" icon="📐" isOpen={openCards.formula} onToggle={() => toggleCard('formula')}>
              <div className="space-y-2">
                <p><strong>When slower crew follows faster:</strong><br/>Start = Previous Start + Buffer</p>
                <p><strong>When faster crew follows slower:</strong><br/>Start = Previous End + Buffer - Duration + 1</p>
              </div>
            </FlashCard>
          </div>

          <div className="bg-white rounded-lg shadow p-4">
            <h4 className="font-bold text-gray-700 mb-3">📊 Your R1 Schedule (Bar Chart)</h4>
            <BarChartR1 schedule={r1Schedule} />
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm">
              <strong>Notice:</strong> This LOB shows your R1 schedule. If there are conflicts (red ✕), you need to adjust start times in R2 below.
            </div>
          </div>
          ...
          <button onClick={handleR2Complete} disabled={!r2Valid} className="w-full py-4 bg-green-600 text-white rounded-lg font-bold text-lg hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed">
            {r2Valid ? 'Complete R2 → R3' : 'Fix buffers and conflicts to continue'}
          </button>
        </>)}

        {/* ROUND 4: R3 BUFFER ANALYSIS */}
        {round === 4 && (<>
          <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded">
            <h3 className="font-bold text-xl text-green-900">📋 Round 3: Buffer Analysis</h3>
            <p className="text-gray-600 mt-1">Explore how changing the buffer affects project duration. Use the slider to experiment.</p>
          </div>
          ...
          <button onClick={handleR3Complete} className="w-full bg-green-600 text-white py-3 rounded-lg font-bold">Complete R3 → R4</button>
        </>)}


        {/* ROUND 5: R4 RATE ANALYSIS */}
        {round === 5 && (<>
          ...
          <button onClick={handleR4Complete} className="w-full bg-green-600 text-white py-3 rounded-lg font-bold">Complete R4 → R5</button>
        </>)}

        {/* ROUND 6: R5 OPTIMIZATION */}
        {round === 6 && (<>
          <div className="bg-purple-50 border-l-4 border-purple-500 p-4 rounded">
            <h3 className="font-bold text-xl text-purple-900">📋 Round 5: Optimization Challenge</h3>
            <p className="text-gray-600 mt-1">Meet both constraints: ≤{TARGET_DAYS} days AND ≤${TARGET_COST.toLocaleString()}</p>
          </div>
